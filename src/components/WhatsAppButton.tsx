...
export default function WhatsAppButton({ currentLang }: { currentLang: Language }) {
  ...
  return (
    <a
      ...
      title={label}
      aria-label={label}
      className="fixed bottom-6 right-6 z-50 flex h-14 w-14 items-center justify-center rounded-full bg-[#25D366] shadow-2xl border-2 border-white transition-all duration-300 hover:scale-110 active:scale-95"
    >
